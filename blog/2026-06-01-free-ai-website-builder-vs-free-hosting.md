...
4. deploy with the W7S GitHub Action;
5. keep future changes in the repository.
...
Paste the workflow from [Deploy From GitHub](/docs/deploy-from-github/) into that file. GitHub Actions runs it when you push to `main`, and W7S uses the repository's GitHub token to deploy that same repository. If the site was generated by a framework, add the build steps shown in [Build before deploy](/docs/deploy-from-github/#build-before-deploy).
