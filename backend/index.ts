...
const STATUS_COMPONENTS: ComponentDefinition[] = [
  ...
  },
  {
    ...
    url: "https://w7s.cloud/api/v1/health",
    expectedStatuses: [200],
...
const componentStatusFor = (params: {
  ...
  const statusMatches = params.definition.expectedStatuses.includes(params.response.status);
...
const checkComponent = async (definition: ComponentDefinition): Promise<StatusComponent> => {
...
export default {
  async fetch(request: Request) {
    ...
    if (request.method === "GET" && url.pathname === STATUS_PATH) {
      return jsonResponse(await statusSummary());
    }
