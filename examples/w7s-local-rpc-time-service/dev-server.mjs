...
const readPort = () => {
  ...
  return Number(process.env.PORT || (index >= 0 ? process.argv[index + 1] : "") || 5178);
};
...
const env = {
  ...
  W7S_REPO: process.env.W7S_REPO ?? "rpc-time",
  W7S_REPOSITORY: process.env.W7S_REPOSITORY ?? "acme/rpc-time",
...
createServer(async (incoming, outgoing) => {
  ...
  console.log(`time service backend listening on http://127.0.0.1:${port}`);
});
