...
Serverless SQL is part of that model. If an app uses a W7S-managed D1 database, database reads, writes, and stored data are included in the W7S usage estimate instead of requiring a separate database bill to understand the real cost shape. External Postgres is still possible when an app needs it.
