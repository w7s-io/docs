...
The calculator estimates monthly W7S usage from traffic, storage, queues, logs, and stateful backend features. It is useful for planning, quota design, and understanding which features multiply cost. It is not a final hosted `w7s.cloud` bill.
...
| Data and object storage | Operation counts and stored GB-month | W7S can provision per-app storage bindings; each capability has its own metered dimensions and rates. |
...
The calculator is a planning tool. It keeps provider details out of the user-facing estimate and shows W7S usage categories instead of infrastructure product names.
...
Use the [Usage Accounting](./usage-accounting.md) API to inspect daily usage, warning thresholds, hard daily caps, hourly resource usage, and the GitHub issue workflow that reports quota pressure back to the repository.
